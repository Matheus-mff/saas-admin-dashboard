    ...
    >
      ...
      <p
        className={cx(
          ...
          hasOnValueChange && "group-hover:text-[var(--foreground)]",
          activeLegend && activeLegend !== name ? "opacity-40" : "opacity-100"
        )}
      >
        {name}
    ...
    <button
      ...
      className={cx(
        ...
        disabled
          ...
          : "cursor-pointer text-[var(--muted-strong)] hover:bg-[var(--hover)] hover:text-[var(--foreground)]"
      )}
      disabled={disabled}
      onClick={(e) => {
        e.stopPropagation();
          ...
          {payload.map(({ value, category, color }, index) => (
            <div key={`id-${index}`} className="flex items-center justify-between space-x-8">
              <div className="flex items-center space-x-2">
                <span
                  aria-hidden="true"
                  className={cx("size-2 shrink-0 rounded-xs", getColorClassName(color, "bg"))}
                />
                <p
                  className={cx(
                    // base
          ...
          >
            {xAxisLabel && (
              <Label
                ...
                offset={-20}
                className="fill-[var(--foreground)] text-sm font-medium"
              >
                {xAxisLabel}
              </Label>
            )}
