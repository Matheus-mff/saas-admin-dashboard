...
const changePasswordSchema = z
  .object({
    ...
    newPassword: z
      ...
      .max(
        MAX_PASSWORD_LENGTH,
        `New password must contain at most ${MAX_PASSWORD_LENGTH} characters.`
      ),
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (!parsedBody.success) {
      return NextResponse.json(
        {
          message: parsedBody.error.issues[0]?.message ?? "Invalid password data.",
        },
        {
          status: 400,
        }
    ...
    if (PROTECTED_DEMO_EMAILS.includes(user.email)) {
      return NextResponse.json(
        {
          message: "Demo account passwords cannot be changed.",
        },
        {
          status: 403,
        }
    ...
    const passwordMatches = await compare(parsedBody.data.currentPassword, user.passwordHash);
    ...
    const newPasswordHash = await hash(parsedBody.data.newPassword, 12);
